// PATCHED COPY of https://telegram.org/js/telegram-widget.js?22 (fetched 2026-07-12).
// 3 changes from upstream, all tagged "patched:" below — route the Login Widget's OAuth traffic
// through our own domain's /tg-oauth proxy instead of oauth.telegram.org directly, because
// Russian ISPs block/throttle the real domain for browsers (the server itself is not blocked).
// See Services/TelegramOAuthProxy.cs and docs/changelog.md for the full story.
// Only the data-telegram-login flow is patched — data-telegram-post/-discussion/-share-url
// (t.me/post.tg.dev) are untouched, since nothing in this codebase uses them.
// If re-vendoring a newer version from Telegram, re-apply these 3 changes by diffing against
// this comment's anchors — do NOT diff mechanically, the upstream file changes over time.
(function(window) {
  (function(window){
  ...
  function initWidget(widgetEl) {
    var widgetId, widgetElId, widgetsOrigin, existsEl,
        ...
        widgetsOriginIsPageOrigin = false; // patched: see postMessageHandler below
    if (!widgetEl.tagName ||
    ...
    if (widgetId = widgetEl.getAttribute('data-telegram-post')) {
      ...
      // patched: route through our own domain's /tg-oauth proxy instead of oauth.telegram.org
      widgetsOrigin = location.origin + '/tg-oauth';
      widgetsOriginIsPageOrigin = true;
    ...
    function postMessageHandler(event) {
      // patched: widgetsOrigin carries a path suffix for the login flow (see above), which
      // postMessage's bare-origin event.origin can never equal — compare against location.origin
      // instead in that case (same-origin proxy: iframe and parent share our own domain).
      if (event.source !== iframe.contentWindow ||
          event.origin != (widgetsOriginIsPageOrigin ? location.origin : widgetsOrigin)) {
        return;
      }
      try {
  ...
  window.Telegram.Login = {
    ...
    widgetsOrigin: location.origin + '/tg-oauth' // patched — see the data-telegram-login branch above
  };
